...
## Harness Engineering
- refactor harness:scaffold skill — needs refactoring to be more about the patterns in a harness than about DDD. need to have example libraries of how to implement in each language. Build out language specific references for java, c#, rust, typescript, go. Patterns include: fitness functions, test layers, acceptance tests, gherkin/bdd and better BDD patterns, arch unit/etc, TDD, context management, skills, hooks, agents, etc.
